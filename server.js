...
mongoose.connect("mongodb://localhost/propertyarticle", {
...
app.get("/scrape", function(req, res) {
  ...
  for (i = 1; i < 2; i++) {
    axios
      .get("https://www.domain.com.au/news/nsw/page/" + i)
      .then(function(response) {
        ...
        $("div.tile-domain-news").each(function(i, element) {
          ...
          // console.log(result);
          writeStream.write(
            `${result.title},${result.publish_date},${result.web_link},${result.image_link} \n`
          );
...
app.get("/articles", function(req, res) {
  ...
  //   var data = {};
  //   db.Article.find({ save_article: false }).then(function(result) {
  //     data.push(result);
  //   });
  ...
  //   db.Article.find({ save_article: false }).then(function(result) {
  //     data.push(result);
  //     db.Note.find({ save_article: false }).then(function(result) {});
  //   });
});

app.get("/article-saved", function(req, res) {
  // Finish the route so it grabs all of the articles
  db.Article.find({ save_article: true }).then(function(data) {
    res.json(data);
  });
});

app.post("/save/article", function(req, res) {
  db.Article.update({ _id: req.body.id }, { save_article: true }, function(
    err,
    res
  ) {}).then(function(data) {
    res.json("it works");
...
app.post("/save/note", function(req, res) {
  ...
  db.Note.create({ articleID: req.body.id }, { note: req.body.note }).then(
    function(note) {
      res.json("it works");
    }
  );
